...
import rawStocksData from "../data/stocks.json";
import { StockSchema } from "../utils/validate";
...
export async function seedStocks(
  ...
  logger.info("Seeding stocks and price history...");

  const stocks = rawStocksData.map((s) => StockSchema.parse(s));
  ...
  for (const stock of stocks) {
    ...
    for (let i = 30; i >= 0; i--) {
      ...
      const volume = Math.max(
        (stockData.volume ?? 10000) * (1 - i * 0.01),
        1000,
      );
  ...
  logger.progress("Stocks", stocks.length);
  logger.progress("Stock price points", stocks.length * 31);
  return stocks;
}
