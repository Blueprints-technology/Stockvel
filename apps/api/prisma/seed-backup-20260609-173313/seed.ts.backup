...
async function main() {
  ...
  const shouldSeed = process.env.ALLOW_SEED === "true" || !isProduction;

  if (isProduction && !shouldSeed) {
    console.log(
      "Production seeding disabled. Set ALLOW_SEED=true to override.",
    );
    return;
  }
  ...
  const demoUser = await prisma.user.upsert({
    where: { email: "demo@stockvel.com" },
    update: {},
    create: {
      email: "demo@stockvel.com",
    ...
    [
      {
        name: "Equities",
        slug: "equities",
        description: "Nigerian stocks and company updates",
      },
      {
        name: "Crypto",
        slug: "crypto",
        description: "Cryptocurrency markets and blockchain insights",
      },
      {
        name: "Economy",
        slug: "economy",
        description: "Macroeconomic developments and policy updates",
      },
      {
        name: "Education",
        slug: "education",
        description: "Financial literacy and explainers",
      },
    ].map((category) =>
  ...
  const sectorCategories = await Promise.all(
    [
      ...
      },
      {
        name: "Consumer Goods",
        slug: "consumer-goods",
        description: "Food, household, and packaged goods companies",
        icon: "ShoppingBasket",
        color: "#ea580c",
      },
      {
        name: "Oil & Gas",
        slug: "oil-gas",
        description: "Energy producers and downstream operators",
        icon: "Fuel",
        color: "#dc2626",
      },
      {
        ...
        description: "Connectivity, towers, and telecom infrastructure",
        icon: "Signal",
        color: "#7c3aed",
      },
      {
        name: "Layer 1",
        slug: "layer-1",
        description: "Core blockchain settlement networks",
        icon: "Cpu",
        color: "#2563eb",
      },
      {
        name: "Stablecoins",
        slug: "stablecoins",
        description: "Dollar-pegged digital assets and settlement rails",
        icon: "Shield",
        color: "#059669",
      },
      {
        ...
        slug: "payments",
        description:
          "Financial rails, merchant services, and payment infrastructure",
        icon: "Wallet",
        color: "#0891b2",
      },
      {
        name: "Commodities",
        slug: "commodities",
        description: "Commodity-linked businesses and inflation hedges",
        icon: "Gem",
        color: "#ca8a04",
      },
    ].map((category) =>
  ...
  const stockProviders = await Promise.all(
    [
      ...
      {
        name: "NGN Market Mirror",
        slug: "ngn-market",
        baseUrl: "https://example.com/ngn-market",
        priority: 3,
        rateLimitPerMin: 20,
      },
      {
        name: "iTick",
        slug: "itick",
        baseUrl: "https://example.com/itick",
        priority: 4,
        rateLimitPerMin: 20,
      },
      {
    ...
    [
      {
        ...
        fetchStrategy: FetchStrategy.RSS,
      },
      {
        name: "Punch Business",
        slug: "punch-business",
        baseUrl: "https://punchng.com/topics/business/",
        fetchStrategy: FetchStrategy.RSS,
      },
      {
        name: "The Guardian Business",
        slug: "guardian-business",
        baseUrl: "https://guardian.ng/category/business-services/",
        fetchStrategy: FetchStrategy.RSS,
      },
      {
        name: "BusinessDay",
      ...
      },
      {
        ...
        fetchStrategy: FetchStrategy.API,
      },
    ].map((source) =>
  ...
  const newsItems = [
    {
      categoryId: newsCategories.find(
        (category) => category.slug === "equities",
      )?.id,
      sourceId: newsSources.find((source) => source.slug === "nairametrics")
        ?.id,
      slug: "nigeria-ftse-russell-return-foreign-inflows",
      title:
        "Nigeria’s FTSE Russell return could trigger fresh foreign inflows",
      excerpt:
        "Analysts expect improved visibility for Nigerian equities after the market re-enters key benchmark conversations.",
      content:
        "Nigeria’s re-entry into global index discussions is expected to improve discoverability, liquidity, and foreign participation across the Nigerian Exchange. This release expands content layering with category browsing, research surfaces, and provider-aware news ingestion.",
      source: "Nairametrics",
    ...
    {
      categoryId: newsCategories.find((category) => category.slug === "crypto")
        ?.id,
      sourceId: newsSources.find((source) => source.slug === "coingecko")?.id,
      slug: "bitcoin-strengthens-above-100k",
      title:
        "Bitcoin strengthens above $100k as institutional appetite returns",
      excerpt:
        "Crypto market breadth broadens with Bitcoin dominance stabilizing and altcoins gaining momentum.",
      content:
        "Bitcoin sustained momentum above the six-figure mark, while Ethereum and Solana also posted healthy gains. The platform uses CoinGecko-backed synchronization for market data and internal curation for market explainers.",
      source: "CoinGecko",
    ...
    },
    {
      ...
      excerpt:
        "A beginner-friendly breakdown of diversification, position sizing, and risk management.",
      content:
        "Investors can blend stable Nigerian dividend stocks with carefully selected crypto assets to create a diversified, multi-asset portfolio. Always review risk tolerance, liquidity, taxes, and time horizon.",
    ...
    {
      categoryId: newsCategories.find((category) => category.slug === "economy")
        ?.id,
      ...
      excerpt:
        "Short-dated instruments remain attractive to conservative allocators watching inflation and naira volatility.",
      content:
        "Treasury yields remain compelling for local investors who want lower volatility instruments while preserving flexibility around duration.",
      source: "BusinessDay",
    ...
    [
      {
        ...
        description: "Foundational investing concepts for beginners",
      },
      {
        name: "NGX Playbook",
        slug: "ngx-playbook",
        description: "How to approach Nigerian equities with clarity",
      },
      {
        name: "Crypto Education",
        slug: "crypto-education",
        description: "Core crypto concepts and risk management",
      },
      {
        name: "Personal Finance",
        slug: "personal-finance",
        description: "Cash flow, saving, and compounding ideas",
      },
    ].map((category) =>
  ...
  const articles = [
    {
      ...
      excerpt:
        "A plain-English guide to dividend dates, yield traps, and total return thinking.",
      content:
        "Dividend investing on the NGX can be rewarding, but investors need to distinguish cash return from real total return. Focus on earnings resilience, board consistency, and payout sustainability.",
      author: "MarketPulse Research Desk",
      categoryId: articleCategories.find((item) => item.slug === "ngx-playbook")
        ?.id,
    ...
    },
    {
      ...
      excerpt:
        "A practical introduction to technical levels and how not to overfit them.",
      content:
        "Support and resistance are useful because market participants cluster around visible levels. Use them with context, not as magic lines.",
      author: "MarketPulse Research Desk",
      categoryId: articleCategories.find(
        (item) => item.slug === "investing-basics",
      )?.id,
    ...
    },
    {
      ...
      excerpt:
        "Why treasury bills matter for capital preservation, yield comparison, and portfolio ballast.",
      content:
        "Treasury bills can serve as a portfolio anchor when risk appetite falls. Compare real yields, tenor risk, and reinvestment assumptions.",
      author: "MarketPulse Research Desk",
      categoryId: articleCategories.find(
        (item) => item.slug === "personal-finance",
      )?.id,
    ...
    },
    {
      ...
      excerpt:
        "How stablecoins are used, and where counterparty and depeg risks can appear.",
      content:
        "Stablecoins improve transfer speed and access, but users should still understand custody, collateral, and platform concentration risks.",
      author: "MarketPulse Research Desk",
      categoryId: articleCategories.find(
        (item) => item.slug === "crypto-education",
      )?.id,
  ...
  const bookmarkedArticle = await prisma.article.findUnique({
    where: { slug: "how-dividend-investing-works-on-the-ngx" },
  });
  ...
  const researchReports = [
    {
      ...
      summary:
        "A concise review of capital, margins, liquidity, and valuation for major listed banks.",
      content:
        "The banking sector remains central to liquidity transmission in Nigeria. Key watchpoints include FX normalization, capital adequacy, and funding cost discipline.",
      reportYear: 2026,
      reportDate: new Date(),
      author: "MarketPulse Research Desk",
    ...
    },
    {
      ...
      summary:
        "An overview of how stablecoins, on-ramps, and exchange access shape market structure.",
      content:
        "Stablecoins continue to play an important role in cross-border value transfer, liquidity buffering, and informal treasury management.",
      reportYear: 2026,
      reportDate: new Date(),
      author: "MarketPulse Research Desk",
    ...
    },
    {
      ...
      summary:
        "How investors can stagger fixed income duration to improve flexibility and reinvestment timing.",
      content:
        "A laddering strategy can reduce reinvestment concentration and allow investors to respond to changing yield conditions with more flexibility.",
      reportYear: 2026,
      reportDate: new Date(),
      author: "MarketPulse Research Desk",
  ...
  const podcasts = [
    {
      ...
      description:
        "A quick market briefing on sector rotation, breadth, and what is driving today’s tape.",
    ...
    },
    {
      ...
      description:
        "A practical conversation about stablecoin utility, payment rails, and risk awareness.",
  ...
  await prisma.newsletter.upsert({
    ...
    update: {
      ...
      content:
        "A concise weekly round-up of NGX, crypto, and research highlights.",
  ...
  for (const provider of stockProviders) {
    await prisma.providerStatusLog
      .create({
        data: {
  ...
  const comment = await prisma.comment
    ...
    .catch(async () =>
      prisma.comment.findFirstOrThrow({ where: { assetSymbol: "ACCESSCORP" } }),
    );
