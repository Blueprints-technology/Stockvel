...
async function main() {
    ...
    if (isProduction && !shouldSeed) {
        console.log("Production seeding disabled. Set ALLOW_SEED=true to override.");
        return;
    }
    ...
    ].map((category) => prisma.category.upsert({
    ...
    const sectorCategories = await Promise.all([
        ...
        },
        {
            name: "Payments",
            slug: "payments",
            description: "Financial rails, merchant services, and payment infrastructure",
            icon: "Wallet",
            color: "#0891b2",
        },
        {
    ...
    ].map((category) => prisma.sectorCategory.upsert({
        where: { slug: category.slug },
        update: category,
        create: category,
    })));
    ...
    ].map((provider) => prisma.stockDataProvider.upsert({
    ...
    const newsSources = await Promise.all([
        {
            ...
            fetchStrategy: client_1.FetchStrategy.RSS,
        },
        {
            name: "Punch Business",
            slug: "punch-business",
        ...
        },
        {
            ...
            fetchStrategy: client_1.FetchStrategy.API,
        },
    ].map((source) => prisma.newsSource.upsert({
    ...
    ].map((category) => prisma.articleCategory.upsert({
        where: { slug: category.slug },
        update: category,
        create: category,
    })));
    ...
    const researchReports = [
        {
            ...
            type: client_1.ResearchReportType.MARKET_COVERAGE,
        ...
        },
        {
            ...
            type: client_1.ResearchReportType.ANALYSIS,
        ...
        },
        {
            ...
            type: client_1.ResearchReportType.STRATEGY,
